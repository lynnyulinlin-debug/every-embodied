...
"""
        ...
        for i, img in enumerate(key_images[-4:]):
            parts.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{image_to_base64(img)}"}})
...
class EmbodiedNavigator:
    ...
    def _render_map_panel(self, state, frontiers=None):
        ...
        if self.last_frontier_mask is not None:
            panel[self.last_frontier_mask > 0] = (255, 255, 0)  # 青色
        ...
        if len(self.traj_pixels) > 1:
            for i in range(len(self.traj_pixels) - 1):
                ...
                cv2.line(panel, p1, p2, (0, 255, 255), 1)  # 黄线
        ...
        if len(self.path_pixels) > 1:
            for i in range(len(self.path_pixels) - 1):
                ...
                cv2.line(panel, p1, p2, (255, 0, 255), 2)  # 洋红
